...
namespace WDA
{
    public partial class FileBorrowStatisticsReport : GridViewUtility
    {
        #region Page_load
        ...
        public void DataBind(bool Anew, bool LockPageNum)
        {
            ...
            try
            {
                ...
                if (Anew)
                {
                    ...
                    if (realName != "選擇人員")
                    {
                        where += string.Format(" AND fb.workerid = N'{0}'", realName.Trim());
                    }

                    string strDept = this.ddlDept.SelectedValue.Trim();
                    if (strDept != string.Empty)
                    {
                        where += string.Format(" AND ut.DEPTID = '{0}'", strDept.Trim());
                    }
                    ...
                    strSql = this.Select.FileBorrowStatistics(where);

                    this.WriteLog(global::Log.Mode.LogMode.DEBUG, strSql);


                    // ADD by Richard 2016/10/01
                    Session["FileBorrowGroup"] = strSql; // used for printing report
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                switch (e.Row.Cells[5].Text)
                {
                    case "N": e.Row.Cells[5].Text = "無";
                        break;
                    case "Y": e.Row.Cells[5].Text = "有";
                        break;
                    default: e.Row.Cells[5].Text = "";
                        break;
                }
